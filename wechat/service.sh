#!/bin/bash
# 微信个人号 Cursor Remote Control 服务管理脚本
...
cmd_stop() {
    ...
    local PIDS=($(pgrep -f "bun run.*wechat/(start|server)" 2>/dev/null || true))
    ...
    # 清理进程锁文件（如果存在）
    rm -f /tmp/cursor-wechat.pid 2>/dev/null || true
...
case "${1:-}" in
    ...
    logs)      cmd_logs ;;
    *)
        echo "微信个人号 Cursor Remote Control 服务管理"
        echo ""
