			...
			if (job.task) {
				switch (job.task.type) {
					...
					case 'fetch-weather': {
						...
						console.log(`[定时任务] 获取天气: ${city}`);
						const card = await fetchWeather({ city, platform: 'wechat' });
						return { status: 'ok' as const, result: card };
					}

					case 'fetch-github-trending': {
						const opts = job.task.options ?? {};
						console.log(`[定时任务] 获取 GitHub Trending: ${opts.since ?? 'daily'}`);
...
`);
	...
	new IdeReplyWatcher("wechat", async (chatId, message) => {
	...
	const monitor = new WechatMonitor(client, async (msg: WeixinMessage) => {
		...
		lastWechatUserId = uid;
			...
			if (pendingFG && !message.trim().startsWith('/') && !CommandHandler.isIdeForwardEnabled(uid)) {
