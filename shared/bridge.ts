 ...
 * 
 * 用法：
 *   bun shared/bridge.ts                 # 直接启动
 *   bun run feishu/bridge.ts             # 通过包装脚本启动
 */
...
function parseEnv(): EnvConfig {
	if (!existsSync(ENV_PATH)) {
		throw new Error(`配置文件不存在: ${ENV_PATH}`);
	}
...
│    model:   cursor-agent
│
│  热更换: 编辑 ${platform}/.env 即可更换 Cursor API Key
└─────────────────────────────────────────────────┘
`);
